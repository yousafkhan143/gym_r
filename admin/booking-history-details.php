...
$idd= $_GET['bookingid'];
// $sqll ="SELECT * FROM tblpayment WHERE bookingID=:bookingid LIMIT 1";
// $state = $dbh->prepare($sqll);
// $Data = [':bookingid'=> $idd];
// $state->execute($Data);
// $result = $state->fetch(PDO::FETCH_OBJ);
$re = $remain - $ParcialPayment;
// echo "remain is ".$remain . "re : ".$re; 
 if ($ParcialPayment > $remain) {
  echo "<script>alert('Payment is greater than Remaining Amount.$ParcialPayment and $remain');</script>";
  // echo "<script>alert('Payment is greater than Remaining Amount.');</script>";
  ...
  // die();
  $sql="INSERT INTO tblpayment(bookingID,paymentType,payment,remaim) Values(:bookindid,:Paymenttype,:ParcialPayment,:remainp);
...
{
  echo "<script>alert('Payment Details Not updated.');</script>";
  echo "<script> window.location.href='booking-history.php';</script>";
}
}
// if($tpay==''):
          ...
          <div class="col-md-12">
           ...
           <div class="col-md-12" id="ParcialPay">
            ...
            if ( $result->remaim==0) {
              ...
              $remain =   $result->remaim;
            }
            
            ?>
            <input type="hidden" name="remain" id="remian" value="<?=$remain;  ?> ">
              <br>
          </div>
