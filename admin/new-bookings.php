...
if (strlen($_SESSION['adminid'] == 0)) {
  ...
  if (isset($_GET['del-booking'])) {

    $bookid = $_GET['del-booking'];
    $sql = "DELETE FROM tblbooking WHERE id=:BookId";
    $query = $dbh->prepare($sql);
    $query->bindParam(':BookId', $bookid, PDO::PARAM_INT);
    $re = $query->execute();
    if ($re) {
      echo "<script>alert('New Booking deleted Successfully');</script>";
      echo "<script>window.location.href='new-bookings.php'</script>";
    } else {
      echo "<script>alert('New Booking Not deleted');</script>";
      echo "<script>window.location.href='new-bookings.php'</script>";
    }
  }
?>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>
                          ...
                          <a onclick=" confirm('Are you sure you want to Delete this Booking?')" href="new-bookings.php?del-booking=<?php echo htmlentities($result->bookingid); ?>"><button class="btn btn-danger" type="button">Delete</button></a>
